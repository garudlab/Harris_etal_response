...
NAm_anc=float(sys.argv[5]) # NEW
NAm_cur=float(sys.argv[6]) # NEW
EUr_anc=float(sys.argv[7]) # NEW
EUr_cur=float(sys.argv[8]) # NEW
...
for i in range(0, numberOfSimulations):
    ...
    Naa = 5224100.0
    Nec = EUr_cur
    Nea = EUr_anc # need to unlog code
    Nnc = NAm_cur 
    Nna =  NAm_anc # need to unlog code
    ...
    if selection == 'True':
        ...
        command='~/./msdir/ms ' + str(totalSampleSize) + ' 1 -t ' +str(theta) +' -I 3 '+ str(sampleSizeAfrica) +' ' + str(sampleSizeEurope) +' ' +str(sampleSizeAmerica) +' -n 2 '+ str(scaledNeEurope) +' -g 2 ' + str(growthRateEurope) + ' -n 3 ' + str(scaledNeAmerica) + ' -g 3 ' + str(growthRateAmerica) 
        #command += ' -m 1 3 ' + str(M_NA) + ' -m 3 1 ' + str(M_NA) + ' -m 2 3 ' + str(M_NE) + ' -m 3 2 ' + str(M_NE)  
