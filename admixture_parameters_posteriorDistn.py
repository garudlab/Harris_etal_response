...
def generate_random_number(inFile):
    # read in the posterior distributions for each parameter
    #distn = open(os.path.expanduser("~/Jensen_response/scripts/posterior_distributions_admixture/distrLogNeAmeBn.txt"), 'r')
...
for i in range(0, numberOfSimulations):
    ...
    Nac=generate_random_number('distrNeAfr.txt')
    ...
    # scaledNeEurope
    Nec=generate_random_number('distrNeEur.txt')
    scaledNeEurope=Nec/Nac

    # Europe ancestral
    Nea_log=generate_random_number('distrLogNeEurBn.txt')
    scaledNeEurope_ancestral=(10**Nea_log)/Nac
    
    # Time of Africa-Europe split
    rand_Tae=generate_random_number('distrLogTimeSplitAfrEur.txt')  
    ...
    Nnc=generate_random_number('distrNeAme.txt')
    scaledNeAmerica=Nnc/Nac

    # admixture time
    Tadm_rand=generate_random_number('distrLogTimeAdm.txt')
    ...
    proportionAdmixture=generate_random_number('distrPropAdm.txt')
    
    # ancestral North America
    Nna=generate_random_number('distrLogNeAmeBn.txt')
    ...
    Ta=generate_random_number('distrTimeAfrBn.txt') # this is already in terms of generations (it has been multiplied by 10)
    ...
    sev=generate_random_number('distrSeverity.txt')
    ...
    Naa=generate_random_number('distrNeAfrAnc.txt')
