...
outFile=~/Jensen_response/analysis/Admixture_mode_varyGrowth_NAm_anc${NAm_anc}_NAm_cur${NAm_cur}_EUr_anc${EUr_anc}_EUr_cur${EUr_cur}_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}_MS

for i in `seq 1 20`; do

echo $i
python ~/Jensen_response/scripts/admixture_parameters_mode_varyGrowth.py $rho_in $adaptive_theta $selection $locusLength $NAm_anc $NAm_cur $EUr_anc $EUr_cur > ${file}_var
