...
peaks=read.table('clusters_082712_400_50_2_2_0_chr2R.txt')
numViz=10
sampleSize=145

numWindowsTotal=length(peaks[,1])
numPanels=round(numWindowsTotal/10)

for (panel in c(1:numPanels)){

outFile=paste('/Users/nanditagarud/Documents/Ace/panelsOfHapSpectra/Chr2R_',panel,'.pdf', sep='')
...
mtext("Position:",side=1,at=-0.8,padj=11.7, cex=cex2)

for (x in c(((panel-1)*numViz+1):(numViz*panel))){
screen(x-(panel-1)*numViz)

# need to adjust the margins
par(mai=c(.1,0,.05,0.03))

# parse the haplotype distribution field of the peaks output.  
a1 <- as.character(peaks[x,3]) 
a2 <- strsplit(a1, ",") 
a3 <- unlist(a2) 
