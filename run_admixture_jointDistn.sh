...
outFile=~/Jensen_response/analysis/Admixture_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}_jointDistn

for i in `seq 1 1000`; do

echo $i
python ~/Jensen_response/scripts/admixture_parameters_jointDistn.py $rho_in $adaptive_theta $selection $locusLength > ${file}_var
