...
outFile=~/Jensen_response/analysis/Admixture_mode_migration_mNA${m_NA}_mNE${m_NE}_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}

for i in `seq 1 40`; do

echo $i
python ~/Jensen_response/scripts/admixture_parameters_mode_migration.py $rho_in $adaptive_theta $selection $locusLength $m_NA $m_NE > ${file}_var
