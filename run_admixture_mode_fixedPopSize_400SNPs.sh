...
outFile=~/Jensen_response/analysis/Admixture_mode_fixedPopSize_NAm${NAm}_EUr${EUr}_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}

for i in `seq 1 100`; do

echo $i
python ~/Jensen_response/scripts/admixture_parameters_mode_fixedPopSize.py $rho_in $adaptive_theta $selection $locusLength $NAm $EUr > ${file}_var
