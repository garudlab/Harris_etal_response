...
def clusterSingleWindow(inFile, outFile, windowTot, distanceThreshold, numStrains, singleWindow, fixedWindow):
# This definiton calculates H12, H2, and H1 in a single window centered around the coordinate specified by the user. If the coordinate is not in the file or within the bounds of a defineable window, then an error message is outputted.  
    ...
    if center!=-1: # this means that the center has not be found
        if fixedWindow ==  'True':
            flies=initialize_fixedWindow(window, center, inFile, coord) 
        else:
...
#######################
def initialize_fixedWindow(window, center, inFile, coord):
# This definition intializes the flies dictionary. Flies takes in the strain number as the key and the haplotype as the value. The flies vector is populated with SNPs read directly from the inFile, using center as a marker as to which SNPs to read in. 
    ...
    coord=int(linecache.getline(inFile,center).split(',')[0])
    window=int(window)
    left_coord=coord-window
    right_coord=coord+window
    print(coord)
    print(window)
    print(left_coord)
    print(right_coord)
    ...
    # left
    i=1
    current_coord=int(linecache.getline(inFile,center-i).split(',')[0])
    while int(current_coord)>=int(left_coord):
        print(current_coord, left_coord)
        for j in range(1,numStrains+1):
    ...
    i=1
    current_coord=int(linecache.getline(inFile,center+i).split(',')[0])
    while int(current_coord)<=int(right_coord):
        print(current_coord, right_coord)
        for j in range(1,numStrains+1):
            flies[j].append(linecache.getline(inFile,center+i).split(',')[j].strip())
        i+=1
        current_coord=int(linecache.getline(inFile,center+i).split(',')[0]) 
...
def mkOptionParser():
    ...
    parser.add_option("-f", "--fixedWindow",      type="string",       default='False',    help="calculate H12 in a fixed window (when single window is defined)")
...
def main():
    ...
    if singleWindow !=-100:
        clusterSingleWindow(inFile, outFile, windowTot, distanceThreshold, numStrains, singleWindow, fixedWindow)
    else:
