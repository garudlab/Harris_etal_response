...
echo $i
python ~/Jensen_response/scripts/admixture_parameters_mode.py $rho_in $adaptive_theta $selection $locusLength > ${file}_var
...
cat ${file}_pi_s_singletons >> ${outFile}_Pi_S_Singletons.txt

python ~/Jensen_response/scripts/Pi_MS.py ${file}_noSingletons ${file}_pi_s_noSingletons

cat ${file}_pi_s_noSingletons >> ${outFile}_Pi_S_noSingletons.txt
