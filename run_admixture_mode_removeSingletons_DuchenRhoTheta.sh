...
outFile=~/Jensen_response/analysis/Admixture_mode_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}
...
python ~/Jensen_response/scripts/admixture_parameters_mode_DuchenRhoTheta.py $rho_in $adaptive_theta $selection $locusLength $mu > ${file}_var
...
python ~/Jensen_response/scripts/Pi_MS.py ${file}_MS ${file}_pi_s_singletons

cat ${file}_pi_s_singletons >> ${outFile}_Pi_S_Singletons_DuchenRhoTheta.txt

python ~/Jensen_response/scripts/Pi_MS.py ${file}_noSingletons ${file}_pi_s_noSingletons

cat ${file}_pi_s_noSingletons >> ${outFile}_Pi_S_noSingletons_DuchenRhoTheta.txt
