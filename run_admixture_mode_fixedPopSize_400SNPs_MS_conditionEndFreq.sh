...
outFile=~/Jensen_response/analysis/Admixture_mode_fixedPopSize_NAm${NAm}_EUr${EUr}_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}_MS_conditionEndFreq
...
# check if freq is in intermediate range:
t=`echo | awk '{ print ('$PF' < 0.99)*('$PF' > 0.01) }'`
