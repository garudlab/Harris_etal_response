...
outFile=~/Jensen_response/analysis/Admixture_posteriorDistn_95CI_corrected_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}_MS

for i in `seq 1 10000`; do

echo $i
python ~/Jensen_response/scripts/admixture_parameters_posteriorDistn_95CI_MS.py $rho_in $adaptive_theta $selection $locusLength > ${file}_var
