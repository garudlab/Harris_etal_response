...
outFile=~/Jensen_response/analysis/Admixture_posteriorDistn_outside_95CI_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}

for i in `seq 1 10000`; do

echo $i
python ~/Jensen_response/scripts/admixture_parameters_posteriorDistn_outside_95CI.py $rho_in $adaptive_theta $selection $locusLength > ${file}_var
...
cat ${file}_pi_s >> ${outFile}_Pi_S_bps.txt
