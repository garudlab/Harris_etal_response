...
outFile=~/Jensen_response/analysis/Admixture_posteriorDistn_95CI_corrected_rho_${rho_in}_theta_${adaptive_theta}_selection_${selection}

for i in `seq 1 10000`; do

echo $i
python ~/Jensen_response/scripts/admixture_parameters_posteriorDistn_95CI.py $rho_in $adaptive_theta $selection $locusLength > ${file}_var
...
cat ${file}_pi_s >> ${outFile}_Pi_S_TajD_bps.txt
